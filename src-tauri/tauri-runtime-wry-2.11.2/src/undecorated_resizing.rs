...
#![cfg(any(windows, target_os = "linux", target_os = "dragonfly", target_os = "freebsd", target_os = "netbsd", target_os = "openbsd"))]
...
mod windows {
    ...
    pub fn attach_resize_handler(hwnd: isize, has_undecorated_shadows: bool) {
        ...
        unsafe {
            ...
            let _ = SetWindowPos(drag_window, Some(HWND_TOP), 0, 0, 0, 0, SWP_ASYNCWINDOWPOS | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOSIZE);
            ...
            let _ = SetWindowSubclass(parent, Some(subclass_parent), (WM_USER + 1) as _, Box::into_raw(Box::new(data)) as _);
        }
    }

    unsafe extern "system" fn subclass_parent(parent: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM, _: usize, data: usize) -> LRESULT {
        match msg {
            WM_SIZE => {
                ...
                if is_maximized(parent).unwrap_or(false) {
                    let _ = SetWindowPos(child, Some(HWND_TOP), 0, 0, 0, 0, SWP_ASYNCWINDOWPOS | SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOMOVE);
                } else {
                    ...
                    if GetClientRect(parent, &mut rect).is_ok() {
                        ...
                        let _ = SetWindowPos(child, Some(HWND_TOP), 0, 0, width, height, SWP_ASYNCWINDOWPOS | SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOMOVE);
        ...
        match msg {
            ...
            WM_NCLBUTTONDOWN => {
                ...
                if res != HitTestResult::NoWhere {
                    ...
                    let _ = PostMessageW(Some(parent), WM_NCLBUTTONDOWN, WPARAM(res.to_win32() as _), LPARAM(&points as *const _ as _));
                }
    ...
    pub fn update_drag_hwnd_rgn_for_undecorated(hwnd: isize, has_undecorated_shadows: bool) {
        ...
        unsafe { SendMessageW(hwnd, WM_UPDATE_UNDECORATED_SHADOWS, None, Some(LPARAM(has_undecorated_shadows as _))) };
        unsafe { SendMessageW(child, WM_UPDATE_UNDECORATED_SHADOWS, None, Some(LPARAM(has_undecorated_shadows as _))) };
    }
...
mod gtk {
    ...
    pub fn attach_resize_handler(webview: &wry::WebView) {
        ...
        webview.add_events(gtk::gdk::EventMask::BUTTON1_MOTION_MASK | gtk::gdk::EventMask::BUTTON_PRESS_MASK | gtk::gdk::EventMask::TOUCH_MASK);
        ...
        //                     let edge = hit_test(0.0, 0.0, window.width() as f64, window.height() as f64, client_x, client_y, border as _, border as _).to_gtk_edge();
        ...
        webview.connect_button_press_event(move |webview: &webkit2gtk::WebView, event: &gtk::gdk::EventButton| {
            if event.button() == 1 {
                if let Some(parent) = webview.parent() {
                    // This one normally should be GtkBox
                    let window = if parent.type_().name() == "GtkBox" {
                        webview.parent().and_then(|w| w.parent())
                    } else {
                        webview.ancestor(gtk::Window::static_type()).and_then(|w| w.parent())
                    };
                    ...
                    if let Some(window) = window.window() {
                        if let Some((root_x, root_y)) = event.root_coords() {
                            if let Some(device) = event.device() {
                                ...
                                match edge {
                                    WindowEdge::__Unknown(_) => (),
                                    _ => window.begin_resize_drag_for_device(edge, &device, 0, root_x as i32, root_y as i32, event.time()),
                                }
                            }
                        }
                    }
