...
pub fn attach_resize_handler(hwnd: isize, has_undecorated_shadows: bool) {
    ...
    // if let Ok(x) = unsafe { FindWindowExW(Some(parent), None, CLASS_NAME, WINDOW_NAME) } {
    //     let _ = unsafe { DestroyWindow(x) };
    // }
        ...
        WM_SIZE => {
            ...
            if is_maximized(parent).unwrap_or(false) {
                ...
                if GetClientRect(parent, &mut rect).is_ok() {
                    ...
                    // set_drag_hwnd_rgn(child, width, 0, has_undecorated_shadows);
                }
            }
        }
